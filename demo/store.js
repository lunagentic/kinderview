...
const dressComment = (c) => {
  const m = member(c.author_slack_user_id);
  return {
    ...c,
    body: c.deleted_at ? '' : c.body,
    author_name: m?.display_name ?? c.author_slack_user_id,
    author_avatar: m?.avatar_url ?? null,
  };
};

function listComments(taskId) {
  return commentsOf(taskId).map(dressComment);
}

function createComment(taskId, input, actor) {
  ...
  let parent = null;
  if (input.parent_id) {
    const pc = (DB.comments ?? []).find((c) => c.id === input.parent_id && c.task_id === taskId);
...
function updateComment(id, input, actor) {
  ...
  if (body.length > 2000) throw new DemoError('코멘트는 2000자까지 쓸 수 있습니다.');
  const at = nowISO();
  Object.assign(c, { body, updated_at: at, edited_at: at });
  save();
  return dressComment(c);
}
