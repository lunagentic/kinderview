...
| 메뉴 | 하는 일 |
|---|---|
| **타임트래킹** | 주간 격자에 시간을 적는다. 프로젝트별·구성원별 투입을 집계하고, 경비(실비)를 등록한다 |
| **인보이싱** | 외주 업체 지급 관리(검수 승인 전에는 지급 완료로 넘어가지 않는다)와 프로젝트별 경비 합계 |
...
| **타임라인** | 프로젝트 × 페이즈 기간을 간트로, 마일스톤을 마름모로. 페이즈·마일스톤을 여기서 관리한다 |
