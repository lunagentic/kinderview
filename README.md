...
정적 배포판은 이제 팀이 같은 데이터를 본다. 표 하나(`kf_rows`)에 개체마다 한 줄이
들어가고, 화면에서 무언가 바뀌면 **바뀐 줄만** 올린다. 규칙과 계산은 그대로 브라우저가
하고, Supabase 는 "같은 것을 본다"만 맡는다. 그래서 라이브러리 없이 REST 로 붙였다 —
설치할 패키지는 여전히 없다.

- 설정은 `demo/store.js` 위쪽의 `SB_URL` · `SB_KEY` 두 줄이다.
  키는 공개용(publishable)이라 브라우저에 내려간다. 접근 범위는 Supabase 의 RLS 가 정한다.
- 저장소가 **비어 있으면** 그때 열려 있던 브라우저의 내용이 공유본이 된다.
  그 뒤로는 저장소에 있는 것이 기준이다.
- 남이 고친 것은 15초마다 확인해 화면을 새로 그린다. 탭이 가려져 있으면 쉰다.
- 저장소에 닿지 못하면 예전처럼 그 브라우저에만 저장하고 계속 돈다.
  상단바에 「공유 중」 / 「이 브라우저에만」으로 지금 상태가 보인다.

지금 정책은 **주소를 아는 사람은 보고 고치기**다. 자물쇠가 아니라 공유 문서에 가깝다.
진짜로 막아야 하면 로그인을 붙여야 한다.
...
| 메뉴 | 하는 일 |
|---|---|
| **인보이싱** | 외주 업체 지급 관리(검수 승인 전에는 지급 완료로 넘어가지 않는다), 프로젝트별 경비 합계와 경비 등록 |
| ~~타임트래킹~~ | 상단 메뉴에서 **숨김**(2026-09-06). 화면·API 는 살아 있어 `#/time` 으로 열린다 |

프로젝트 매니징 아래 2차 탭:

| 화면 | 하는 일 |
|---|---|
| **타임라인** | **프로젝트 매니징을 열면 여기가 먼저 나온다** (2차 탭은 끌어서 순서를 바꿀 수 있고, 첫 칸이 진입 화면이다). 프로젝트 × 페이즈 기간을 간트로, 마일스톤을 마름모로. 페이즈·마일스톤을 여기서 관리한다 |
| **Overview** | 맨 앞에 **보드 지도** — 업무 하나가 타일 하나. 단계(열) · 프로젝트(색) · 지연(빨간 테두리)을 한눈에 본다. 이어서 위험 신호, 프로젝트별·업무 영역별 진행(영역 리드 표시), 외주 진행 현황. 모든 숫자와 타일에서 원본 업무로 내려간다 |
...
| 13 | [타임트래킹·인보이싱](docs/13-time-invoice-spec.md) | 메뉴 재편, 시간 기록, 경비, 외주 지급 |
| 14 | [타임라인](docs/14-timeline-spec.md) | 페이즈, 간트, 마일스톤, 프로젝트 색 규칙 |
